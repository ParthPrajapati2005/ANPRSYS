...
###################### EXISTING CODE USED FROM EXTERNAL SOURCE############################
                                                                                                 #Raw Detections
@tf.function
def detect_fn(image):
...
###################### EXTERNAL CODE ENDS HERE############################
        ...
        print(detected_registration_plate)
...
#while cap.isOpened():  
while plateDetected == False:                                                                    #While the video source is open do this
    ret, frame = cap.read()
    ...
    viz_utils.visualize_boxes_and_labels_on_image_array(                                         #Call tensorflow viz.utils function - this visualises and draws the bozes on detections in real time
        ...
        agnostic_mode=False)
    numberPlate = detect_text(image_np_with_detections)
    
    if numberPlate != None:
        print(numberPlate)
        plateDetected = True  
                                                                                                 #Call text detection function with ROI
    cv2.imshow('object detection', cv2.resize(image_np_with_detections, (800, 600)))             #Open the window

    if cv2.waitKey(10) & 0xFF == ord('q'):                                                       #Keyboard Interrupt
        cap.release()
